        ...
        <?php else: ?>
            <?php foreach ($products as $product): ?>
                <div class="product-card fade-in">
                    <img src="<?php echo BASE_URL; ?>images/products/<?= htmlspecialchars($product['image']) ?>"
                         alt="<?= htmlspecialchars($product['name']) ?>">
                    <h3><?= htmlspecialchars($product['name']) ?></h3>
                    <p class="price">$<?= number_format($product['price'], 2) ?></p>
                    <a href="product.php?id=<?= (int)$product['id'] ?>" class="btn">Ver Detalles</a>
                    <a href="cart.php?add=<?= (int)$product['id'] ?>" class="btn">Añadir al Carrito</a>
                </div>
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
